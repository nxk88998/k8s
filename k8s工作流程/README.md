...
ControllerManager -> kubeapiserver : 监测发现新的deployment
...
scheduler -> kubeapiserver : 通过list-watch机制,监测发现新的pod,经过主机过滤主机打分规则,将pod绑定(binding)到合适的主机.

kubelet -> kubeapiserver : 通过NodeName 获取自身Node上所要运行的pod清单.通过与自己的内部缓存进行比较,新增加pod.
